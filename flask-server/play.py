...
def wordle_bot(addr, port, local):
    ...
    offical_results = get_pattern(data)
    store_results(offical_results, data, word_data)
    print(answer)
    print(offical_results)
    post_results(offical_results)
    driver.close()

def post_results(results):
    from_id = "bot@wordle.brsoft.io"
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    print(f"{from_id} {timestamp} {results}")
    url = "https://wgrp.brsoft.io/group/callback"          
